...
def recentUsers():
  try:
    cursor.execute("set local timezone to 'CST'; Select first_name,last_name,phone,age(now(),clock_in),email, to_char(clock_in, 'HH12:MI') FROM attendance,users WHERE attendance.user_id = users.id AND attendance.clock_out IS NULL ORDER BY  duration DESC ;")

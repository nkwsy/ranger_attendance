...
def recentUsers():
  try:
    cursor.execute("Select first_name,last_name,phone,age(now(),clock_in),email, to_char(timezone(CST, clock_in), 'HH12:MI') as duration FROM attendance,users WHERE attendance.user_id = users.id AND attendance.clock_out IS NULL ORDER BY  duration DESC ;")
